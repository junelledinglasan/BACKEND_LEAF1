...
def overview_view(request):
    ...
    total_releases = Loan.objects.filter(status__in=['Active','Completed']).aggregate(t=Sum('amount'))['t'] or 0
    total_paid     = Payment.objects.aggregate(t=Sum('amount'))['t'] or 0
    rate           = round((float(total_paid)/float(total_releases))*100, 1) if total_releases else 0
    ...
    return Response({
        ...
        'total_releases':   float(total_releases),
        'avg_loan_amount':  float(Loan.objects.aggregate(a=Avg('amount'))['a'] or 0),
        'total_collection': float(total_paid),
...
def monthly_collection_view(request):
    if request.user.role not in ['admin','staff']:
        return Response({'error':'Unauthorized.'}, status=403)
    data = (Payment.objects.annotate(month=TruncMonth('paid_at')).values('month').annotate(total=Sum('amount')).order_by('month'))
    return Response([{'month': d['month'].strftime('%b %Y'), 'total': float(d['total'])} for d in data])
